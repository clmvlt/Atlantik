...
namespace Atlantik.Formulaires
{
    public partial class FormAjouterSecteur : Form
    {
        ...
        {
            if (tbxNomSecteur.Text == String.Empty)
            {
            ...
            {
                maCnx.Open();
                var maCde = new MySqlCommand("insert into secteur (nom) values (@NOMSECTEUR)", maCnx);
                maCde.Parameters.AddWithValue("@NOMSECTEUR", tbxNomSecteur.Text);

                maCde.ExecuteNonQuery();
                MessageBox.Show("Secteur Ajouté!", "Atlantik Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Close();
            }
